...
export const helper = () => {
  ...
  const renderTodos = (_todos) => {
    if (document.getElementById('todo-names') != null) {
      document.getElementById('todo-names').remove();
    }
    codefactory('projectNames', { class: 'border border-gray-200 my-3 p-5 rounded-lg flex flex-col divide-gray-200', id: 'todo-names' }, '', 'todo-flex-container')
    if (_todos){
